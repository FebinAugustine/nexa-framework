import { renderPage } from './framework';
import { DEFAULT_SECURITY_HEADERS, isSafePath } from './security';
...
export async function handleRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (path === '/__nexa_proxy') {
      try {
        const { service, method, args }: { service: string; method: string; args: any[] } = await req.json();
